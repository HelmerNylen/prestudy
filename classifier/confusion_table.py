...
class ConfusionTable:
	...
	def __map_indices(self, true_labels, confused_labels):
		...
		elif confused_labels is None:
			second = None
		else:
			raise IndexError(f"Only strings or string slices are allowed (got: {repr(confused_labels)})")
	...
	def __str__(self):
		...
		for row, true_label in enumerate(self.true_labels):
			res.append("  ".join([true_label.rjust(widths[0])] + [f"{val:>{widths[i+1]}.2%}" for i, val in enumerate(self.data[row])]))
