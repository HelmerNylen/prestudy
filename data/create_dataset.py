...
def sph2pipe(kaldi_folder: str, input_file: str):
	"""Converts a sphere file to a wave file understood by pydub"""
	...
	if args.list:
		...
		for t, speech_t, noise_t, output_t in [
			...
			for i in range(len(speech_t)):
				# Load speech file
				audio = load_audio(speech_t[i], lambda input_file: sph2pipe(args.kaldi, input_file), matlab_engine)
...
if __name__ == "__main__":
	...
	group.add_argument("--kaldi", help="Path to kaldi root folder (default: %(default)s)", default=os.path.join("..", "kaldi"))
	...
	group = parser.add_argument_group("output folders")
	...
	group = parser.add_argument_group("degradation parameters")
	...
	parser.add_argument("-c", "--classes", help="The class types to use in addition to silence (default: all)", nargs="+", default=[])
	parser.add_argument("--train", help="Ratio/number of files in training set (default: %(default).2f)", default=11/15, type=float)
	parser.add_argument("--test", help="Ratio/number of files in testing set (default: %(default).2f)", default=4/15, type=float)
	parser.add_argument("--profile", help="Profile the dataset creator", action="store_true")
	parser.add_argument("--no-cache", help="Disable caching noise files. Increases runtime but decreases memory usage.", action="store_true")

	parser.add_argument("-l", "--list", action="store_true", help="List available files and degradations")

	args = parser.parse_args()
	if args.profile:
		import cProfile
		cProfile.run("main(args)", sort="cumulative")
	else:
		start = time()
		main(args)
